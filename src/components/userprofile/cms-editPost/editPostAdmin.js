...
function EditPostAdmin(props) {
    ...
    // console.log(Boolean(defPost.featureFlag))
    // let data = useLocation();
    ...
    useEffect(()=>{
        ...
        Axios.get('https://api-dev.trustnews.ca/getArticleDropdowns').then(response =>{
            ...
            Axios.get(`https://api-dev.trustnews.ca/editArticle?postID=${data.state.postID}`).then(response =>{
                ...
                // console.log(response.data[0].breakingFlag)
                setBreakingD(Boolean(response.data[0].breakingFlag))
                setFeatureD(Boolean(response.data[0].featureFlag))
    ...
    const onSubmit = dat => {
      ...
      articleCom.featureFlag = featureD
      articleCom.breakingFlag = breakingD
      ...
      articleCom.statusID = '1'
        ...
        if(defPost.featureFlag){
            if(articleCom.featureFlag){
                articleCom.featureExp = defPost.featureExp
            }else{
        ...
        let todayC = new Date()
        articleCom.committedOn = todayC.getFullYear()+'-'+(todayC.getMonth()+1)+'-'+todayC.getDate();
        if(defPost.breakingFlag){
            
            if(articleCom.breakingFlag){
                articleCom.breakingTimestamp = defPost.breakingTimestamp
            }else{
        ...
        //     articleCom.topicTypeID = data.state.topicTypeID
        // }
        Axios.post('https://api-dev.trustnews.ca/updateArticleAdmin',articleCom).then(response =>{
        ...
        <div className='cms-compose-body'>
            ...
            compose-post">
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='compose-request'>
                    <label>feature</label>
                    <input name='featureFlag' type="checkbox" defaultChecked={featureD} onClick={handleClickF}/>

                    <label>breaking</label>
                    <input name='breakingFlag' type="checkbox" defaultChecked={breakingD} onClick={handleClickB}/>
                </div> 
